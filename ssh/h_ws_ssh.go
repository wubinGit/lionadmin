...
func WsSsh(c *gin.Context,manchine model.TMachine) {

	/**
	校验参数
	 */
	//param := c.Param("username")
	//if strings.Compare(param,"wxlf")!=1 {
	//	fmt.Println("user password ")
	//	return
	//}
	/**
	[{"key":"Upgrade: websocket\nConnection: Upgrade","value":"","description":"","type":"text","enabled":true}]
	 */
	...
	client, err := NewSshClient(manchine)
	if wshandleError(wsConn, err) {
		return
	}
	defer client.Close()
