...
func SubStr(str string,aim string) (result int) {
    index := strings.Index(str, aim)
    return index

}
func SubString(str string,begin int,end int) (result string)  {
    strarr:=[]rune(str)
    length :=len(str)
    if begin<0 {
        begin=0
    ...
    return string(strarr[begin:end])
}
